...
`;
    ...
    if (history.length > 0) {
      historyPrompt = `
【对话历史】：
${history.map(m => `${m.role === 'user' ? '管理者' : '扫地僧'}: ${m.content}`).join('\n')}
`;
    }

    const prompt = `你是一个资深的实战派企业管理顾问（人称"扫地僧"）。
...
请使用纯文本格式，适当换行。`;
    ...
    return response.text || '抱歉，扫地僧正在闭关，请稍后再试。';
  } catch (error) {
    console.error('Gemini API Error:', error);
    return '抱歉，扫地僧的通讯线路受到干扰，请稍后再试。';
  }
}
